...
public abstract class AbstractDeployableDescriptorFactory<M extends MuleDeployableModel, T extends DeployableArtifactDescriptor>
  ...
  public T create(File artifactFolder) throws ArtifactDescriptorCreateException {
    ...
    final File mulePluginJsonFile = new File(artifactFolder, MULE_ARTIFACT_FOLDER + separator + getDescriptorFileName());
    if (!mulePluginJsonFile.exists()) {
      throw new IllegalStateException("Artifact descriptor does not exists: " + mulePluginJsonFile);
    }
    artifactDescriptor = loadFromJsonDescriptor(artifactFolder, mulePluginJsonFile);
  ...
  protected T loadFromJsonDescriptor(File artifactFolder, File artifactJsonDescriptor) {
    ...
    // TODO(pablo.kraan): domains - creaet method for this
